...
# want columns: jw_dist_last_name, jw_dist_forename, match_city, match_state, ft_dist_last_name
# get return from training_data in distances.py 

class ReusableClassifier():  # Camel case, because it's a class, matches filename
    def __init__(self, model_dir: str = 'data/cc.en.50.bin', model_type: str = 'xgb'):  # maybe a base directory?
        self.model = (self._initialize_xgb_model() if model_type == 'xgb' 
                      else self._initialize_random_forests())
        self.ft_model = fasttext.load_model(model_dir)
